...
Runtime values are read from ``config/config.yaml`` when present.  The module
falls back to sensible defaults so it can still be imported when the config
file is missing (e.g. during isolated unit tests).
"""
...
def _get_config_value(config: dict, candidate_paths: tuple, default: Any) -> Any:
